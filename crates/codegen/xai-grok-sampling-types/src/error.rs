    ...
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            SamplingError::Api { status, .. } => {
                // 408: our fork treats request timeout as retryable (often oversized payload)
                // 529: upstream added Cloudflare "Site is overloaded"
                matches!(status.as_u16(), 408 | 429 | 500 | 502 | 503 | 504 | 520 | 529)
            }
