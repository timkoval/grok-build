...
///
/// Fork extension: also treat HTTP 408 Request Timeout as retryable — often
/// signals an oversized payload / proxy timeout that may succeed on retry.
pub fn is_retryable_api_status(status: StatusCode) -> bool {
    status.as_u16() == 408 || RetryPolicy::edge_client().should_retry(status.as_u16())
}
